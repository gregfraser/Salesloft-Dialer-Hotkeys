...
node behaviour.mjs   # the Not in Service flow, the pane, the not-armed state (73)
...
name between four places, `listPage` drops the contact marker,
`downshiftMenu` keeps the disposition menu in the DOM while it is closed, and
`queue` puts other people's identical remove controls ahead of the contact's,
and `hoverReveal` draws a row's control only while that row is hovered. See
...
- `not-in-service` takes a different branch of the same UI, and its steps come from a recording of the flow done by hand rather than from guesswork: the split button's caret (`[data-testid="menuToggle"]`, accessible name "Open Log only or complete only menu") → the menu's "Log Only" → the cadence's own control, matched on the accessible name "Remove person from cadence" because it is an icon button with no text. **That control is not unique, and the one clicked must be the contact's.** The task queue beside a contact carries an identical one on every row, one per person, and the flow used to take the first on the page: Salesloft answered "Task removed for" someone else. The queue also draws a row's control **only while that row is hovered**, so the single control on the page was whichever row the rep's pointer rested on, which is how the first-on-the-page lookup removed someone else. So the flow first hovers every place the contact's name is written (`revealRowsFor()`, hover events only, nothing clicked), and `removalFor()` then accepts a control only when the contact's name (`slContactName()`: the tab title "Corey Adamonis | People | Salesloft", else the heading's leaf; shared with `alerts.js` via `defaults.js`) is in the control's **row**: the first container around it holding any text of its own (`hasTextBeyond()`), and never anything further up. A looser rule, "climb until another remove control", was merged once and was wrong: with only the hovered row's control on the page, the climb reached the whole queue, found the contact's row in it, and took another person's control for theirs. The climb still stops at another control, a landmark (the heading, the logger, the plate) or 800 characters of text. Controls count whether shown or not, since a hover-revealed one may be hidden. Exactly one must qualify: none and two are both a stop, and a miss logs the name and each control's row text (`dumpRemoval()`). It is resolved **before anything is touched** (no call ended, nothing logged) and again right before the click, because logging re-renders the queue. Row text is read with its text nodes joined by spaces (`spacedText()`), since `textContent` runs "Targeting" into "Eric" and a glued name is not a whole word. A confirmation dialog may or may not follow; its *absence* is a normal outcome (hence `CONFIG.confirmTimeout`, 1.5s, not the 8s step timeout), but a dialog that appears with no recognised button throws rather than being left open. Two rules make that work and both were bugs first: the dialog is found with `isShown()` rather than `visible()`, because **`offsetParent` is null for a `position:fixed` element** and every modal is one — `visible()` stays correct for the controls *inside* a container and wrong for the container itself; and only a dialog that was **not already open** when the removal was clicked counts, because Salesloft's own logger popout carries `role="dialog"` and a document-wide lookup matched the popout the flow had just been driving.
...
- `killAndLog` sets the disposition **before** clicking "Log & Complete". Any failed step throws, surfaces "Stopped: … Finish manually.", and leaves the call unlogged — never log with a wrong or missing disposition. `runNotInService` keeps the same rule and adds two: the cadence removal goes **last**, because a person removed from a cadence with no call logged against them is the worse half-state of the two; and **it never removes anyone but the person on screen**. It runs only on the contact's own page (`onPersonPage()`: the route or the person-detail marker, never the logger popout, because dialled from a list the heading is the list's name and a row mentioning it would read as that person's). Whose page it is and which one control is theirs are settled before the call is ended, so a page the flow cannot read stops clean with nothing logged, and the control is found again, under the same rule, before it is clicked. Removing the wrong person is the one failure here that the rep may not notice and cannot easily undo, so any doubt is a stop.
