...
**`extension/defaults.js` is the single definition** — loaded as a plain script by the content script (via the manifest), the service worker (`importScripts`), and both HTML pages (`<script>` before their own). Add new settings there only; do not reintroduce per-file copies. It also holds what the panel and the on-page transcript both need to agree on — the alert palette and headlines, and the transcript formatters (`slFormatClock`, `slTranscriptText`, `slTranscriptFilename`) — so a line reads the same in either and a saved file does not depend on which one wrote it. `SL_HEADLINE` says what to **do** about a tag, never what the tag already says: both surfaces render the headline next to the tags themselves, so a headline that paraphrases one ("Meeting already scheduled — Meeting Scheduled") burns the banner's only line on the same sentence twice. Its `module.exports` is what makes those testable under `node --test`.
...
- **The overlay never moves or resizes with its content.** It is pinned bottom-left; the button column is a fixed 240px wide, the transcript pane a fixed 300px, and pane, rail and column are all the same fixed `PANEL_HEIGHT`, so transcript lines scroll and the status clamps (two lines, tooltip for the rest) rather than pushing anything around. Inside the column the two action buttons and the status beneath them add up to exactly `PANEL_HEIGHT`, and the status's height is **reserved, not measured** — that is what stops a two-line "Stopped: …" from shoving the buttons upward. The buttons stretch **vertically** to meet the pane and never widen: the pair keeps the column's width and splits it, so their left/right positions are identical with the transcript on, off or folded away. The contact alert is the one part that comes and goes on its own, so it is a full-width banner **above** that row — the overlay is anchored at its foot, so the banner grows the box upward and moves nothing (`width:0;min-width:100%` stops a long headline deciding the overlay's width). Hover and press effects are `filter`/`transform` only, for the same reason. Nothing else resizes it but a deliberate click: transcription on or off (which adds the pane and the taller buttons), and minimising the pane (width only).
