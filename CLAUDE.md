...
- **The overlay never moves or resizes with its content.** It is pinned bottom-left; the button column is a fixed 240px, the transcript pane a fixed 300×152, and both columns are the same fixed `PANEL_HEIGHT`, so status and alert text clamp (two lines each, tooltip for the rest) and transcript lines scroll rather than pushing anything around. Inside the column, the two action buttons and the foot beneath them add up to exactly that height, and the foot's height is **reserved, not measured** — that is what stops a two-line contact alert from shoving the buttons upward when it appears. Hover and press effects are `filter`/`transform` only, for the same reason. The overlay's height never changes at all now; its width changes for exactly two reasons, both a deliberate click: transcription being switched on or off (which adds or removes the pane), and the rep minimising the pane. Anything that makes it grow with what it is displaying re-introduces the jumping the fixed sizes exist to stop.
