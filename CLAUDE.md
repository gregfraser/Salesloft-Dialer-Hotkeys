...
because it caught a bug that the unit tests and a screenshot both missed:
`decoys` fills the feed with pills whose entire text is the disposition being
looked for, `listPage` drops the contact marker, and `downshiftMenu` keeps the
disposition menu in the DOM while it is closed. See
...
the same functions. The shipped bindings are two arrows under one hand, left
and right. A bound arrow stops scrolling the Salesloft page while the overlay
is loaded, since the in-page handler `preventDefault`s whatever it matches.
Empty stays a normal value, and a rep who wants the key back clears it in the
popup. Storage is synced, so an older install's `not-in-service` binding (the
up arrow) can still be there; `slNormalizeHotkeys()` reads only the actions
that exist, so it does nothing and the page keeps its scrolling.
...
- **A timeout names what it was waiting for.** `waitFor(fn, timeout, interval, what)` — `Stopped: could not find "No Answer" in the Disposition list` is a bug report; "Timed out waiting for element" is not.
...
- **The plate can be dragged, but it still never moves or resizes with its *content*.** It starts
  ...
  the same line and transcript lines scroll rather than push anything around. The status sits on
  the **base row** under the pair, across the whole plate — a sentence reads better across it than down 214px —
