...
`chrome.storage.sync` holds dialer settings (`floatingPanel`, `pageOverlay`, `disposition`), contact-alert settings (`alertsEnabled`, `alertTags`, `alertStrict`) and transcription settings (`transcription`, `outputDeviceId`, `serverUrl`, `healthUrl`). With `transcription` on, auto-start and transcript saving are unconditional — they are behaviours, not settings. `transcription` also decides whether the on-page overlay carries the transcript pane, so toggling it rebuilds the overlay.

**Saving a transcript is triggered once, by the worker, after the flush.** `stopTranscription()` waits for the server to drain the queue and only then calls `requestTranscriptSave()` — saving on the call-end `call-state` message instead would cut the call's last utterance. That one call picks a single surface: the floating panel if it is open (`save-transcript`, and it writes the file itself), otherwise the on-page pane (`transcript-unsaved`, and it *offers* the save). Both hold the same lines, so asking both would write the call twice.

The asymmetry is a Chrome rule, not a preference: a web page gets one uninvited download before Chrome starts asking the user's permission for the rest, so an automatic per-call save from `content.js` would put a permission bubble on the Salesloft page partway through a call block. Extension pages are under no such limit. `content.js` therefore only ever downloads from a click on its ↓ button.
